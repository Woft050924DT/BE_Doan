...
import { resolveVariantId } from './stock';
...
const saleNotes = (applied: boolean) =>
  applied
    ? `Xuất kho khi giao hàng thành công ${STOCK_APPLIED_MARKER}`
    : 'Xuất kho khi giao hàng thành công';

const isStockApplied = (notes: string | null | undefined) =>
  Boolean(notes?.includes(STOCK_APPLIED_MARKER));
...
export const deductStockForOrder = async (
  ...
  for (const item of items) {
    ...
    const variantId = await resolveVariantId(item.product_id, item.variant_id);
