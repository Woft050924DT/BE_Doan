...
exports.receiveStock = exports.adjustStock = exports.getInventoryTransactions = exports.getInventoryList = exports.getInventorySummary = exports.LOW_STOCK_THRESHOLD = void 0;
...
const ensureUniqueSlug = async (tx, baseSlug) => {
    let slug = baseSlug;
    let counter = 1;
...
const getInventorySummary = async () => {
    ...
    const lowStock = variants.filter((v) => {
        ...
        return q > 0 && q <= exports.LOW_STOCK_THRESHOLD;
    }).length;
    return {
        totalSkus,
        totalStock,
        outOfStock,
        lowStock,
        lowStockThreshold: exports.LOW_STOCK_THRESHOLD,
    };
};
exports.getInventorySummary = getInventorySummary;
const buildVariantWhere = (filters) => {
    ...
    if (status === 'out') {
        ...
        where.stock_quantity = { gt: 0, lte: exports.LOW_STOCK_THRESHOLD };
    }
    else if (status === 'in_stock') {
        where.stock_quantity = { gt: exports.LOW_STOCK_THRESHOLD };
    }
    return where;
};
const getInventoryList = async (filters) => {
    ...
    const [items, total] = await Promise.all([
        database_1.default.product_variants.findMany({
            where,
            skip,
            take,
            include: {
        ...
        database_1.default.product_variants.count({ where }),
    ]);
    return {
        items,
        pagination: {
...
const getInventoryTransactions = async (filters) => {
    ...
    const [transactions, total] = await Promise.all([
        database_1.default.inventory_transactions.findMany({
            where,
            skip,
            take,
            include: {
        ...
        database_1.default.inventory_transactions.count({ where }),
    ]);
    return {
        transactions,
        pagination: {
...
const adjustStock = async (userId, data) => {
    ...
    return database_1.default.$transaction(async (tx) => {
        ...
        const transaction = txnQuantity > 0
...
const receiveStock = async (userId, data) => {
    ...
    return database_1.default.$transaction(async (tx) => {
        const variant = await tx.product_variants.findUnique({
            where: { variant_id },
            include: {
                products: { select: { product_id: true, name: true, price: true, cost_price: true } },
            },
        });
        if (!variant?.products) {
            throw new Error('Variant not found');
        ...
        (0, pricing_1.assertValidSellingPrice)(sellPrice, unit_cost);
        const current = variant.stock_quantity ?? 0;
...
const receiveNewProductStock = async (userId, data) => {
    ...
    (0, pricing_1.assertValidSellingPrice)(Number(price), unit_cost);
    const existingSku = await database_1.default.products.findUnique({ where: { sku: sku.trim() } });
    if (existingSku)
        throw new Error('SKU already exists');
    const variantSku = `${sku.trim()}-STD`;
    const existingVariantSku = await database_1.default.product_variants.findUnique({
    ...
    return database_1.default.$transaction(async (tx) => {
        ...
        const imageUrls = product.image_urls?.map((u) => u.trim()).filter(Boolean) || [DEFAULT_IMAGE];
